...
// 2. A wrapper that implements isKarlsendMessage_Payload, having a single field pointing to the concrete command
//    bearing the name of the command with `KarlsendMessage_` prefix and `Request` suffix
...
// generateKarlsendMessage generates a wrapped KarlsendMessage with the given `commandValue`
func generateKarlsendMessage(commandValue reflect.Value, commandDesc *commandDescription) (*protowire.KarlsendMessage, error) {
	commandWrapper := reflect.New(commandDesc.typeof)
	unwrapCommandValue(commandWrapper).Set(commandValue)

	karlsendMessage := reflect.New(reflect.TypeOf(protowire.KarlsendMessage{}))
	karlsendMessage.Elem().FieldByName("Payload").Set(commandWrapper)
	return karlsendMessage.Interface().(*protowire.KarlsendMessage), nil
}
